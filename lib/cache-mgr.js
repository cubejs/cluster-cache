...
module.exporots = {
    ...
    'configApp': function configApp(app) {
        var basePort = process.env.CACHE_BASE_PORT || 9190;
        return pickAvailablePorts(basePort, basePort + 100, 2).then(function (ports) {
            logger.info('[cache-mgr] pick up ports %j', ports);
            manager.ports = ports;
