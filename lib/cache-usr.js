...
exports.use = function(domain){

	if(!process.userPromise){//each process needs at most one cache user

		logger().info('[cache] user created');

		common.domainPath().then(function(domainPath){

			logger().info('[cache] domain path resolved:%s', domainPath);

			tillExists(domainPath);

			user.switchDomain(domain || fs.readFileSync(domainPath, {'encoding':'utf-8'}));
		});
